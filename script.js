...
const QuizApp = (function() {
    'use strict';
    // ===================================
    // SMART PASTE LOGIC (THÊM MỚI)
    // ===================================

    function processSmartPaste() {
        const text = document.getElementById('smartPasteInput').value.trim();
        
        if (!text) {
            showToast('⚠️ Vui lòng dán nội dung câu hỏi vào ô trống!', 'error');
            return;
        }
        ...
        try {
            ...
            if (questions.length === 0) {
                throw new Error('Không nhận diện được câu hỏi nào. Hãy kiểm tra lại định dạng!');
            }
            ...
            // Build map cho tính năng tra cứu
            state.questionMap.clear();
            state.allQuestions.forEach(q => state.questionMap.set(q.id, q));
            
            // Cập nhật bộ lọc chủ đề
            updateTopicFilter();
            
            // Reset các settings cũ và bật nút bắt đầu
            document.getElementById('startBtn').disabled = false;
            
            // Tự động bắt đầu thi luôn cho tiện
            startQuiz();
            showToast(`✅ Đã tạo thành công ${questions.length} câu hỏi!`);
